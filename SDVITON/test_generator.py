...
def test(opt, test_loader, board, tocg, generator):
    ...
    gauss = GaussianBlur((15, 15), (3, 3)).cuda()
    
    tocg.cuda(); tocg.eval()
    generator.eval()
    
    output_dir = opt.output_dir if opt.output_dir is not None else os.path.join(
        './output', opt.test_name, opt.datamode, opt.datasetting, 'generator', 'output')
    ...
    with torch.no_grad():
        for inputs in test_loader.data_loader:
            ...
            warped_cm_onehot = torch.FloatTensor(
                (warped_clothmask_taco.detach().cpu().numpy() > 0.5).astype(np.float64)).cuda()
            ...
            flow_tvob = F.interpolate(
                flow_list_tvob[-1].permute(0, 3, 1, 2),
                size=(iH, iW), mode='bilinear'
            ).permute(0, 2, 3, 1)
            flow_tvob_norm = torch.cat([flow_tvob[:, :, :, 0:1] / ((flow_iW - 1.0) / 2.0),
                                        flow_tvob[:, :, :, 1:2] / ((flow_iH - 1.0) / 2.0)], 3)
            ...
            flow_taco = F.interpolate(
                flow_list_taco[-1].permute(0, 4, 1, 2, 3), 
                size=(2, iH, iW), mode='trilinear'
            ).permute(0, 2, 3, 4, 1)
            flow_taco_norm = torch.cat([
                flow_taco[:, :, :, :, 0:1] / ((flow_iW - 1.0) / 2.0),
                flow_taco[:, :, :, :, 1:2] / ((flow_iH - 1.0) / 2.0),
                flow_taco[:, :, :, :, 2:3]
            ], 4)
            warped_cloth_tvob = warped_cloth_tvob.unsqueeze(2)
            warped_cloth_taco = F.grid_sample(
                torch.cat((warped_cloth_tvob, torch.zeros_like(warped_cloth_tvob).cuda()), dim=2),
                flow_taco_norm + grid_3d, padding_mode='border')
            warped_cloth_taco = warped_cloth_taco[:, :, 0, :, :]

            warped_clothmask_tvob = warped_clothmask_tvob.unsqueeze(2)
            warped_clothmask_taco = F.grid_sample(
                torch.cat((warped_clothmask_tvob, torch.zeros_like(warped_clothmask_tvob).cuda()), dim=2),
                flow_taco_norm + grid_3d, padding_mode='border')
            ...
            # === BLUR THE FINAL OUTPUT IMAGE BEFORE SAVING ===
            # Loop over output, blur each image before saving to disk
            unpaired_names = []
            for i in range(shape[0]):
                # BLUR just the output image (the try-on result)
                blurred_output = kornia.filters.gaussian_blur2d(
                    output[i].unsqueeze(0), kernel_size=(7, 7), sigma=(2.5, 2.5)
                ).squeeze(0)
                # You may also blur 'grid' if you want grid visualizations to be smoother:
                grid = make_image_grid([
                    (clothes[i].cpu() / 2 + 0.5),
                    (pre_clothes_mask[i].cpu()).expand(3, -1, -1),
                    visualize_segmap(parse_agnostic.cpu(), batch=i),
                    ((densepose.cpu()[i] + 1) / 2),
                    (warped_cloth_taco[i].cpu().detach() / 2 + 0.5),
                    (warped_clothmask_taco[i].cpu().detach()).expand(3, -1, -1),
                    visualize_segmap(fake_parse_gauss.cpu(), batch=i),
                    (pose_map[i].cpu() / 2 + 0.5),
                    (warped_cloth_taco[i].cpu() / 2 + 0.5),
                    (agnostic[i].cpu() / 2 + 0.5),
                    (im[i] / 2 + 0.5),
                    (blurred_output.cpu() / 2 + 0.5)  # Notice: use blurred output in final grid
                ], nrow=4)
                unpaired_name = (
                    inputs['c_name']['paired'][i].split('.')[0]
                    + '_' + inputs['c_name'][opt.datasetting][i].split('.')[0] + '.png'
                )
                save_image(grid, os.path.join(grid_dir, unpaired_name))
                unpaired_names.append(unpaired_name)
            
            # Save blurred individual try-on output images
            blurred_output_batched = torch.stack([
                kornia.filters.gaussian_blur2d(img.unsqueeze(0), (7, 7), (2.5, 2.5)).squeeze(0)
                for img in output
            ])
            save_images(blurred_output_batched, unpaired_names, output_dir)
